...
def display_words(page_num):
    try:
        ...
        canvas.config(background=bgColor,width=350,height=500)
...
def search_word():
    try:
        ...
        if results:
            for word in results:
                for english_word, meaning in word.items():
                    word_label = tk.Label(result_frame, text=f"{english_word}: {meaning}", anchor='w', justify='left', wraplength=360,background=bgColor,font=('맑은 고딕',14))
                    word_label.pack(anchor="w", padx=10, pady=5)
        else:
    ...
    try:
        ...
        if words:
            ...
            for page_num in range(1, num_pages + 1):
                page_button = customtkinter.CTkButton(button_frame, text=f"Part {page_num}",width=180, height=40,font=("맑은 고딕",16 ,"bold"),bg_color=bgColor,fg_color=fgColor,hover_color=hoverColor, command=lambda num=page_num: display_words(num))
                page_button.pack(side="top", pady=17)
