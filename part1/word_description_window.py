...
class DescriptionWindowApp:
    ...
    def open_description_window(self):
        # 새로운 창 생성
        description_window = customtkinter.CTkToplevel(self.parent_window)
        
        description_window.title("단어장 설명")
        description_window.geometry("400x500")
        description_window.resizable(0, 0)
        description_window.config(background=self.bgColor)

        title_label = tkinter.Label(description_window, text="TOEICVOCAMACA", width=20, height=2, font=("맑은 고딕", 24, "bold"),background=self.bgColor)
        title_label.pack()
        ...
        close_button = customtkinter.CTkButton(description_window, text="닫기", command=description_window.destroy,bg_color=self.bgColor,fg_color=self.fgColor,hover_color=self.hoverColor)
